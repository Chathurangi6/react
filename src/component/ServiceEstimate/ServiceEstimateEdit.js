...
const TITLE = 'Edit Service Estimate'
class ServiceEstimateEdit extends React.Component {
  ...
  emptyStay={
    ...
    async componentDidMount() {
      ...
      await fetch(`/spestedy/findAppointmentIdByStatusAndDeptId?status=V&deptId=${this.state.idGroup.costCenter}`)
      ...
      if(this.props.location.state){
        ...
        idGroup.estimationNo=this.props.location.state.estimationNo
        this.setState({idGroup,isParams:true},()=>this.getData());
        this.props.history.replace('/serviceEstimate/edit/',null);
      }
  ...
  setStaysToTable(list){
    ...
    async handleSubmit(event){
      ...
      this.setState({nullList:[],nullValues:false});
      if(this.checkNullValues()){
        this.setState({nullValues:true});
      }else{
      ...
      const response=await fetch('/serviceEstimate/edit', {
        method: 'POST',
        headers: {
    ...
    checkNullValues(){
      let nullList=this.state.nullList;
      nullList=[];
      let serviceEstimate=this.state.serviceEstimate;
      let nullValues=false;
      if(serviceEstimate.serviceLength==""){
        nullList[0]=0;
        nullValues=true;
      }
      if(serviceEstimate.distanceToSp==""){
        nullList[1]=0;
        nullValues=true;
      }
      if(serviceEstimate.sin==""){
        nullList[2]=0;
        nullValues=true;
      }
      if(serviceEstimate.noOfSpans==""){
        nullList[3]=0;
        nullValues=true;
      }
      if(serviceEstimate.insideLength==""){
        nullList[4]=0;
        nullValues=true;
      }
    ...
    render() {    
      ...
      <Helmet>
        ...
        <a className="path2" href="/serviceEstimate/edit"> Edit Service Estimate</a>
        </div>
          <Container>
        <h1 align="center">Edit Service Estimate</h1>
        <br></br>
        <ul id="progressbar">
        ...
        <div className="row">
            ...
            <button className="btn btn-primary float-right" type="button" style={{marginBottom:'10px'}} onClick={this.handleSubmit} disabled={!this.state.searched}>Save Service Estimate</button>
            </div>
          </div>
          {(()=>{
          if(this.state.nullValues==true){
            return(

              <div class="alert alert-danger" role="alert">
                Please fill the fields below
                <div>
                {(()=>{if(this.state.nullList[0]==0){return(<span>&emsp; &#8226;Service Wire Length</span>)  }})()}
                {(()=>{if(this.state.nullList[1]==0){return(<span>&emsp; &#8226;Distance to Service Place</span>)  }})()}
                {(()=>{if(this.state.nullList[2]==0){return(<span>&emsp; &#8226;SIN Number</span>)  }})()}
                {(()=>{if(this.state.nullList[3]==0){return(<span>&emsp; &#8226;No of Spans</span>)  }})()}
                {(()=>{if(this.state.nullList[4]==0){return(<span>&emsp; &#8226;Length Inside the premises</span>)  }})()}
                </div>
  ...
  function Step1(props) {
    const {handleChange,idGroup, applicant, application, wiringLandDetail,estimateNoList,nextStep,isParams}=props;
...
export default ServiceEstimateEdit;