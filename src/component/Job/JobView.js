...
class JobView extends React.Component {
  ...
  // }
    ...
    //   this.handleSubmit=this.handleSubmit.bind(this);
      ...
      this.undoRejectJob=this.undoRejectJob.bind(this); 
      this.rejectJob=this.rejectJob.bind(this);
      this.approveJob=this.approveJob.bind(this);
    ...
    async componentDidMount() {
      ...
      switch(this.props.status){
        ...
        //   break;
        case "undoRejects":
          status=estimateStatus.JOB_REJECTED;
          break;
        ...
        case "approveLowerLevel":{
          ...
          await fetch(`/job/getProjectNoApprovalListLowerLevels?userName=${userName}&authorityLevel=${userLevel}`)
          ...
          // {
          //   if(userRole=="ES"){
          //     status=estimateStatus.JOB_APPROVAL_ES;
          //   }else if(userRole=="EA"){
          //     status=estimateStatus.JOB_APPROVAL_EA;
          //   }else if(userRole=="EE"){
          ...
          //     status=estimateStatus.JOB_APPROVAL_PEN;
          //   }else if(userRole=="PCE"){
          //     status=estimateStatus.JOB_APPROVAL_CEC;
          //   }
          // }
      }
      if(status!=0){
        if(this.props.location.state){
          ...
          await fetch(`/pcesthmt/findProjectNoByStatusAndDeptIdAndCatCd?status=${status}&deptId=${this.state.idGroup.costCenter}&catCd=${this.state.idGroup.catCd}`)
          .then(response => response.json())
          .then(data => this.setState({jobNoList: data.sort()}));
        }  
      }
      // console.log(this.state.jobNoList.sort())
      // this.setState({jobNoList});
  ...
  async loadData(status){
    ...
    let idGroup=this.state.idGroup;
    idGroup.jobNo=this.props.location.state.jobNo;
    this.setState({idGroup},()=>this.getData());
    if(this.props.status=="undoRejects"){
      this.props.history.replace('/job/undoRejects/',null);
    }else if(this.props.status=="approveOrReject"){
      this.props.history.replace('/job/approveOrReject/',null)
    }
  ...
  handleChangeIdGroup(event) {
    ...
    let idGroup = {...this.state.idGroup};
    idGroup[name] = value;
    if(status=="view"||status=="cancel"||status=="deactivate"||status=="activate"){
      if(idGroup.jobNo.length!=18){
        event.target.setCustomValidity('Please lengthen the job no to 18 characters');
    ...
    //   idGroup.costCenter=idGroup.jobNo.substring(0,6);
    //   this.setState({idGroup},()=>this.getData());
    // }
  }
  ...
  setActions(e){
    ...
    if(status=="view"){
      ...
      this.searchForDeactivate();
    }else if(status=="activate"){
      this.searchForActivate();
    }
  }

  async searchForCancel(){
    let status= await fetch(`/pcesthmt/findStatusByProjectNo?projectNo=${this.state.idGroup.jobNo}`);
    if(status.status==200){
      status=await (status).json();
      if(status==estimateStatus.JOB_APPROVED){
        let approvedBy= await(await fetch(`/approval/findLastApprovedBy?toStatus=${estimateStatus.JOB_APPROVED}&referenceNo=${this.state.idGroup.jobNo}`)).json();
  ...
  async searchForDeactivate(){
    let status= await fetch(`/pcesthmt/findStatusByProjectNo?projectNo=${this.state.idGroup.jobNo}`);
    if(status.status==200){
      status=await (status).json();
      if(status==estimateStatus.JOB_POSTED){
        this.getData();
      }else{
        this.clearForm();
        Swal.fire(
          'Error!',
          "Job can't be deactivated. Job should be in posted state.",
          'error'
        )
      }
    }else{
  ...
  async getData(){
    if(this.state.idGroup.jobNo){
      ...
      if(application.status==200){
        ...
        // let pcesthmt = await fetch(`/pcesthmt/findByIdEstimateNoAndIdDeptIdAndIdRevNo?estimateNo=${this.state.idGroup.estimationNo}&deptId=${this.state.idGroup.costCenter}&revNo=1`);
        // if(pcesthmt.status==200){
        //   pcesthmt=await (pcesthmt).json();
        //   this.setState({pcesthmt},()=>console.log(this.state.pcesthmt));
        // }else{
  ...
  async deactivateJob(){
    ...
    if(message.value){
      const {idGroup,standardEstimate,pcesthmt}=this.state
      const approval={
        approvalId:'',
        referenceNo:idGroup.jobNo,
        deptId:pcesthmt.id.deptId,
        approvalType:'JOB_DEACTIVATE',
        systemBy:'SMC',
        approvedLevel:sessionStorage.getItem('userLevel'),
        approvedBy:sessionStorage.getItem('userName'),
        fromStatus:estimateStatus.JOB_POSTED,
        toStatus:'0',
        standardCost:'',
        detailedCost:''
      }
      ...
      const response=await fetch('/job/changeStatusAndAddApproval', {
        method: 'POST',
        headers: {
  ...
  async activateJob(){
    ...
    if(message.value){
      ...
      const approval={
        ...
        referenceNo:idGroup.jobNo,
        deptId:pcesthmt.id.deptId,
        approvalType:'JOB_ACTIVATE',
        systemBy:'SMC',
        approvedLevel:sessionStorage.getItem('userLevel'),
        approvedBy:sessionStorage.getItem('userName'),
        fromStatus:'0',
        toStatus:estimateStatus.JOB_POSTED,
        standardCost:'',
        detailedCost:''
      }
      //const response = await fetch(`/estimate/delete?estimateNo=${this.state.idGroup.estimationNo}&deptId=${this.state.idGroup.costCenter}&revNo=1`);
      const response=await fetch('/job/changeStatusAndAddApproval', {
        ...
        body: JSON.stringify({
          estimateNo:pcesthmt.id.estimateNo.trim(),
          deptId:pcesthmt.id.deptId,
          status:estimateStatus.JOB_POSTED,
          approval:approval}),
      });
      if(response.status==200){
        this.clearForm();
  ...
  async undoRejectJob(){
    ...
    const message= await Swal.fire({
      ...
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      confirmButtonText: 'Yes, activate rejected job!'
    })
    if(message.value){
      const {idGroup,standardEstimate,pcesthmt}=this.state
      const approval={
        ...
        referenceNo:idGroup.jobNo,
        deptId:pcesthmt.id.deptId,
        approvalType:'JOB_UNDO_RJCT',
        systemBy:'SMC',
        approvedLevel:sessionStorage.getItem('userLevel'),
        approvedBy:sessionStorage.getItem('userName'),
        fromStatus:estimateStatus.JOB_REJECTED,
        toStatus:estimateStatus.JOB_RIVISION,
        standardCost:'',
        detailedCost:''
      }
      //const response = await fetch(`/pcesthmt/updateStatus?estimateNo=${this.state.idGroup.estimationNo}&deptId=${this.state.idGroup.costCenter}&revNo=1&status=75`);
      const response=await fetch('/job/changeStatusAndAddApproval', {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
        ...
        body: JSON.stringify({
          estimateNo:pcesthmt.id.estimateNo.trim(),
          deptId:pcesthmt.id.deptId,
          status:estimateStatus.JOB_RIVISION,
          approval:approval}),
      });
      ...
      if(response.status==200){
        ...
        await fetch(`/pcesthmt/findProjectNoByStatusAndDeptIdAndCatCd?status=${estimateStatus.JOB_REJECTED}&deptId=${this.state.idGroup.costCenter}&catCd=${this.state.idGroup.catCd}`)
          .then(response => response.json())
          .then(data => this.setState({jobNoList: data.sort()}));
  ...
  async rejectJob(){
    ...
    if(message.value){
      ...
      const approval={
        ...
        referenceNo:idGroup.jobNo,
        deptId:pcesthmt.id.deptId,
        approvalType:'JOB_RJCT',
        systemBy:'SMC',
        approvedLevel:userLevel,
        approvedBy:userName,
        fromStatus:this.state.pcesthmt.status,
        toStatus:estimateStatus.JOB_REJECTED,
        standardCost:'',
        detailedCost:''
      }
      //const response = await fetch(`/pcesthmt/updateStatus?estimateNo=${this.state.idGroup.estimationNo}&deptId=${this.state.idGroup.costCenter}&revNo=1&status=75`);
      ...
      if(response.status==200){
        ...
        if(status=="approveOrReject") {   
          await fetch(`/pcesthmt/findProjectNoByStatusAndDeptIdAndCatCd?status=${this.getStatusForUserRole()}&deptId=${this.state.idGroup.costCenter}&catCd=${this.state.idGroup.catCd}`)
          .then(response => response.json())
          .then(data => this.setState({jobNoList: data.sort()}));
  ...
  async approveJob(){
    ...
    if(message.value){
      ...
      var response;
      if(status=="approveOrReject"){
        response = await (await fetch(`/job/approve?jobNo=${this.state.idGroup.jobNo}&estimateNo=${this.state.pcesthmt.id.estimateNo.trim()}&deptId=${this.state.pcesthmt.id.deptId}&userLevel=${userLevel}&userName=${userName}`)).text();
      }else if(status=="approveLowerLevel"){
        response = await (await fetch(`/job/approveLowerLevel?jobNo=${this.state.idGroup.jobNo}&estimateNo=${this.state.pcesthmt.id.estimateNo.trim()}&deptId=${this.state.pcesthmt.id.deptId}&userLevel=${userLevel}&userName=${userName}`)).text();
      }
  ...
  async cancelJob(){
    ...
    const message= await Swal.fire({
      ...
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      confirmButtonText: 'Yes, cancel approved job!'
    })
    if(message.value){
      const {idGroup,standardEstimate,pcesthmt}=this.state
      const approval={
        ...
        referenceNo:idGroup.jobNo,
        deptId:pcesthmt.id.deptId,
        approvalType:'JOB_CLAP',
        systemBy:'SMC',
        approvedLevel:sessionStorage.getItem('userLevel'),
        approvedBy:sessionStorage.getItem('userName'),
        fromStatus:estimateStatus.JOB_APPROVED,
        toStatus:estimateStatus.JOB_RIVISION,
        standardCost:'',
        detailedCost:''
      }
  ...
  clearForm = () =>{
    ...
    getNameOfView(){
      ...
      switch(status){
        ...
        case "deactivate": return <span>Deactivate </span>
        case "activate": return <span>Activate </span>
    ...
    setActionButton(){
      ...
      switch(status){
        ...
        case "deactivate": return <button className="btn btn-danger float-right" type="button" style={{marginBottom:'10px'}} onClick={this.deactivateJob} disabled={!this.state.searched}>Deactivate Job</button>
        case "activate": return <button className="btn btn-primary float-right" type="button" style={{marginBottom:'10px'}} onClick={this.activateJob} disabled={!this.state.searched}>Activate Job</button>
        case "undoRejects": return <button className="btn btn-primary float-right" type="button" style={{marginBottom:'10px'}} onClick={this.undoRejectJob} disabled={!this.state.searched}>Activate Rejected Job</button>
        case "approveOrReject": 
        case "approveLowerLevel":
          return <div className="float-right">
            <button className="btn btn-primary" type="button" style={{marginBottom:'10px',marginRight:'20px'}} onClick={this.approveJob} disabled={!this.state.searched}>Approve Job</button>
            <button className="btn btn-primary" type="button" style={{marginBottom:'10px'}} onClick={this.rejectJob} disabled={!this.state.searched}>Reject Job</button>
          </div>
        case "cancel": return <button className="btn btn-primary float-right" type="button" style={{marginBottom:'10px'}} onClick={this.cancelJob} disabled={!this.state.searched}>Cancel Approved Job</button>
      }
    }
      ...
      return (
        <React.Fragment>
          <div>                  
                         ...
                         <a className="path" href="/home">  Manage Jobs  </a> |
                         <a className="path2" href="/home"> {this.getNameOfView()} Job </a> 
  ...
  function Step1(props) {
    const {handleChange,idGroup,getData,setActions,applicant, application, wiringLandDetail,jobNoList,pcesthmt,nextStep,status,toInputUppercase,searched,isParams}=props;
              ...
              return(
                ...
                <Form onSubmit={setActions}>
                  ...
                  <div className="col-sm-3">
                    <Input type="text" name="jobNo" id="jobNo" value={idGroup.jobNo} onInput={toInputUppercase}
                           minLength="18" maxLength="18" onChange={handleChange} autoComplete="jobNo" required/>
                  </div>
                  <div className="col-sm-3">
  ...
  class Step4 extends React.Component {
    ...
    render() {    
      const {currentStep,standardEstimate,pcesthmt,nextStep,prevStep}=this.props;
      ...
      return (
        ...
        <br></br>
        <div style={{color:'blue'}}>Estimated with <b>{pcesthmt.estimatedYear}</b> Standard Contruction Rates</div>
  ...
  class Step6 extends React.Component {
    ...
    render() {    
      const {currentStep,labourCostList,prevStep}=this.props;
