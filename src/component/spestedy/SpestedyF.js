...
const TITLE = 'Failed/Active Appoinment'
...
console.log(this.state.toSendStatus)
       this.state.toSendStatus.map(mapTo=>{
        ...
        fetch(`spestedy/changeStatusToA?applicationNo=${mapTo.applicationNo}`,{
          method:'GET',
          headers: {
          ...
          Swal.fire({    
              type: 'success',
              title: 'Application Activated Sucessfully',
    ...
    getDetail(){
        ...
        fetch(`/spestedy/findByDates?allocatedDate=${correctDate}`)
  ...
  render() {
   ...
   return (
      <div>
        ...
        <a className="path2" href="/spestedf"> Failed/Active</a>
     </div>
         <Helmet>
          <title>{ TITLE }</title>
        </Helmet>
      <br></br>
      <h3 align="center">Failed /  Active Appoinments </h3>
