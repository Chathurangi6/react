...
class EstimateAdd extends React.Component {
  ...
  // }
    ...
    constructor(props) {
      ...
      this.handleChangeServiceEstmate=this.handleChangeServiceEstmate.bind(this);

    }

    async componentDidMount() {
      
      await fetch(`/spestedy/findAppointmentIdByStatusAndDeptId?status=V&deptId=${sessionStorage.getItem('costCenterNo')}`)
  ...
  async getLabourRates(){
    ...
    await fetch(`/labour/findByIdDeptIdAndIdYear?deptId=${this.state.idGroup.costCenter}&year=${year}`)
    .then(response => response.json())
  ...
  async getMaterials(){
    await fetch(`/inwrhmtm/getMaterials?warehouse=${this.state.idGroup.costCenter}`)
    .then(response => response.json())
...
handleChangeServiceEstmate(event) {
    const target = event.target;
    const value = target.value;
    const name = target.name;
  ...
  async handleSubmit(event){
    ...
    if(message.value){
    const { serviceEstimate,idGroup,materialCostList, labourCostList} = this.state;
    ...
    const standardEstimate={
      ...
      serviceLength:serviceEstimate.serviceLength,
      conductorType:serviceEstimate.bareconType,
      conductorLength:serviceEstimate.bareconLength,
      span:serviceEstimate.noOfSpans,
      insideLength:serviceEstimate.insideLength,
      poleNo:serviceEstimate.poleno,
      lineLength:serviceEstimate.totalLength,
      conversionLength:serviceEstimate.conversionLength,
      conversionLength2p:serviceEstimate.conversionLength2p,
      secondCircuitLength:serviceEstimate.secondCircuitLength,
      wiringType:serviceEstimate.wiringType,
      isLoopService:serviceEstimate.loopCable,
      cableType:serviceEstimate.cableType,
      isStandardVc:serviceEstimate.isStandardVc,
      stayNo:'',
      strutNo:'',
      fixedCost:0,
      variableCost:0,
    ...
    const response=await fetch('/estimate/add', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        pcestdtt:selectedMaterialCosts,
      ...
      return (
        <React.Fragment>
          ...
          <Step2 
            ...
            handleChange={this.handleChangeServiceEstmate}
            nextStep={this.nextStep}
  ...
  function Step1(props) {
    const {handleChange,idGroup,applicant, application, wiringLandDetail,estimateNoList,pcesthtt,nextStep,status,toInputUppercase,searched,isParams}=props;
    ...
    return(
      <div>
        ...
        <div className="form-group row">
          <div className="col-sm-4 col-form-label">Cost Center : {idGroup.costCenter}</div>
  ...
  function Step2(props) {
    const {wiringLandDetail,serviceEstimate,handleChange,nextStep,prevStep}=props;
          ...
          <div className="col-sm-3 col-form-label">
            <div className="form-check form-check-inline">
              <input className="form-check-input" type="radio" name="wiringType" id="overHead" value="Over Head"
                     checked={serviceEstimate.wiringType==="Over Head"}  onChange={handleChange}/>
          ...
          <div className="col-sm-3 col-form-label">
            <div className="form-check form-check-inline">
              <input className="form-check-input" type="radio" name="loopCable" id="yes" value="Yes"
                     checked={serviceEstimate.loopCable==="Yes"}  onChange={handleChange}/>
              <label className="form-check-label" for="yes">Yes</label>
            </div>
            <div className="form-check form-check-inline">
  ...
  function Step3(props) {
    const {handleChange,serviceEstimate,standardEstimate,nextStep,prevStep}=props;
  ...
  class Step4 extends React.Component {
    ...
    render() {    
      const {currentStep,serviceEstimate,standardEstimate,pcesthtt,nextStep,prevStep,handleChange,getTotalCostForNBT,getTotalCostForVAT,getEstimatedTotalCost}=this.props;  
      ...
      return (
        ...
        <div style={{color:'blue'}}>Variable cost is calculated using <b>{serviceEstimate.isStandardVc=="Yes"?"Standard":"Non Standard"}</b> Method</div>
        <br></br>
