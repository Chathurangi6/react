...
class EstimateEdit extends React.Component {
  ...
  // }
    ...
    async componentDidMount() {
      // const estimateNoList=["514.20/ENC/19/0001","514.20/ENC/19/0002","514.20/ENC/19/0080","514.20/ENC/19/0090"];
      // this.setState({estimateNoList});
      await fetch(`/pcesthtt/findEstimateNoByStatusAndDeptIdAndRevNo?status=${estimateStatus.MODIFIED}&deptId=${sessionStorage.getItem('costCenterNo')}&revNo=1`)
  ...
  async getLabourRates(){
    ...
    await fetch(`/labour/findByIdDeptIdAndIdYear?deptId=${this.state.pcesthtt.id.deptId}&year=${year}`)
    .then(response => response.json())
  ...
  async getMaterials(){
    await fetch(`/inwrhmtm/getMaterials?warehouse=${this.state.pcesthtt.id.deptId}`)
    .then(response => response.json())
  ...
  async handleSubmit(event){
    ...
    if(message.value){
    const { standardEstimate,idGroup,pcesthtt,materialCostList, labourCostList} = this.state;
    ...
    const response=await fetch('/estimate/edit', {
      method: 'POST',
      headers: {
  ...
  function Step1(props) {
    ...
    return(
      <div>
        ...
        <div className="form-group row">
          <div className="col-sm-4 col-form-label">Cost Center : {pcesthtt.id.deptId}</div>
  ...
  class Step4 extends React.Component {
    ...
    render() {    
      const {currentStep,standardEstimate,pcesthtt,nextStep,prevStep,handleChange,getTotalCostForNBT,getTotalCostForVAT,getEstimatedTotalCost}=this.props;  
